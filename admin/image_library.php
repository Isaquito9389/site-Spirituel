...
function updateImageLibraryTable($pdo) {
    try {
        ...
        // Vérifier si la colonne ritual_id existe
        $stmt = $pdo->query("SHOW COLUMNS FROM image_library LIKE 'ritual_id'");
        if ($stmt->rowCount() == 0) {
            $pdo->exec("ALTER TABLE image_library ADD COLUMN ritual_id INT DEFAULT NULL");
            // Ajouter un index pour améliorer les performances
            $pdo->exec("ALTER TABLE image_library ADD INDEX idx_ritual_id (ritual_id)");
        }
...
// Récupérer la liste des rituels pour la sélection
$rituals = [];
try {
    $stmt = $pdo->prepare("SELECT id, title FROM rituals ORDER BY title ASC");
    $stmt->execute();
    $rituals = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    // Ignorer l'erreur si la table rituals n'existe pas encore
}

// Lire les images de la base de données avec les informations des rituels
$images = [];
try {
    // Vérifier si la colonne ritual_id existe avant de faire le JOIN
    $stmt = $pdo->query("SHOW COLUMNS FROM image_library LIKE 'ritual_id'");
    $ritual_id_exists = $stmt->rowCount() > 0;
    
    if ($ritual_id_exists) {
        // Si la colonne existe, faire le JOIN avec les rituels
        $stmt = $pdo->prepare("
            SELECT il.*, r.title as ritual_title 
            FROM image_library il 
            LEFT JOIN rituals r ON il.ritual_id = r.id 
            ORDER BY il.uploaded_at DESC
        ");
    } else {
        // Si la colonne n'existe pas encore, récupérer seulement les images
        $stmt = $pdo->prepare("
            SELECT *, NULL as ritual_title 
            FROM image_library 
...
<head>
    ...
    <style>
        ...
        .drop-zone {
            ...
            text-align: center;
            transition: all 0.3s ease;
            background: linear-gradient(145deg, rgba(26, 26, 46, 0.5), rgba(58, 12, 163, 0.1));
        }
        
        .drop-zone:hover, .drop-zone.dragover {
            border-color: var(--accent);
            background: linear-gradient(145deg, rgba(26, 26, 46, 0.8), rgba(247, 37, 133, 0.1));
...
<body class="bg-gray-900 text-white min-h-screen">
            ...
            <form method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-xs text-gray-500">
                        Formats supportés: JPG, PNG, GIF, WEBP - Max: 6MB
                    </div>
                </div>
                ...
                <div class="bg-gray-700 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label for="ritual_id" class="block text-sm font-medium text-gray-300 mb-2">
                                <i class="fas fa-magic mr-1"></i>Associer à un rituel
                            </label>
                            <select name="ritual_id" id="ritual_id" 
                                    class="w-full px-3 py-2 bg-gray-600 border border-gray-500 rounded-lg focus:ring-2 focus:ring-purple-500 text-white">
                                <option value="">Aucun rituel</option>
                                <?php foreach ($rituals as $ritual): ?>
                                    <option value="<?php echo $ritual['id']; ?>">
                                        <?php echo htmlspecialchars($ritual['title']); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <div class="text-xs text-gray-500 mt-1">
                                Optionnel : Associez cette image à un rituel spécifique
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <?php foreach ($images as $image): ?>
                        <div class="image-item">
                            ...
                            <div class="image-actions">
                                <!-- Bouton de sélection pour les popups -->
                                <button class="action-btn bg-green-600 hover:bg-green-700" onclick="selectImage('<?php echo htmlspecialchars($image['image_path']); ?>', '<?php echo htmlspecialchars($image['original_name'] ?? basename($image['image_path'])); ?>')" title="Sélectionner cette image" style="display: none;" id="select-btn-<?php echo $image['id']; ?>">
    ...
    <script>
        ...
        // Fonction pour sélectionner une image (utilisée dans les popups)
        function selectImage(imagePath, imageName) {
            ...
            if (window.opener && window.opener !== window) {
                ...
                // Si ce n'est pas une popup, copier l'URL
                copyToClipboard(imagePath);
            }
        }

        // Détecter si nous sommes dans une popup et afficher les boutons de sélection
        document.addEventListener('DOMContentLoaded', function() {
            const isPopup = window.opener && window.opener !== window;
            
            if (isPopup) {
                // Afficher tous les boutons de sélection
                const selectButtons = document.querySelectorAll('[id^="select-btn-"]');
                selectButtons.forEach(button => {
                    button.style.display = 'flex';
                });
                
                // Modifier le titre de la page
                document.title = 'Sélectionner une image - Bibliothèque d\'images';
                
                // Ajouter un indicateur visuel
                const header = document.querySelector('header .text-gray-300');
                if (header) {
                    header.textContent = 'Bibliothèque d\'images - Mode sélection';
                }
                ...
                if (mainContent) {
                    ...
                    instructionDiv.className = 'mb-6 p-4 rounded-lg bg-blue-800 text-blue-100 border border-blue-600';
                    instructionDiv.innerHTML = `
                        <div class="flex items-center">
                            <i class="fas fa-info-circle mr-2"></i>
                            <span>Cliquez sur le bouton vert <i class="fas fa-check"></i> pour sélectionner une image</span>
                        </div>
                    `;
                    mainContent.insertBefore(instructionDiv, mainContent.firstChild.nextSibling);
                }
            }
        });
    </script>
